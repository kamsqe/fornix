...
function runStatus(args = ""): string {
  return execSync(`${CLI} status ${args}`, {
    cwd: TEST_DIR,
    encoding: "utf-8",
    timeout: 10000,
  });
...
function runStatusSafe(
  cwd: string,
  args = "",
): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync(`${CLI} status ${args}`, {
      cwd,
      encoding: "utf-8",
      timeout: 10000,
...
describe("fornix status", () => {
  ...
  it("outside a fornix project shows helpful error message", () => {
    const nonProjectDir = join(tmpdir(), "fornix-no-project-" + Date.now());
    mkdirSync(nonProjectDir, { recursive: true });

    const result = runStatusSafe(nonProjectDir);
